...
if [ -z "$1" ]; then
          echo "Error: Please provide a Username for SFTP authentication."
          exit 1
fi
...
aws cloudformation create-stack --stack-name clamav-scan --template-body file://anti-virus-scan-template.yaml --parameters ParameterKey=CodeBuildStack,ParameterValue=clamav-codebuild ParameterKey=UserName,ParameterValue=$1 --capabilities CAPABILITY_NAMED_IAM
